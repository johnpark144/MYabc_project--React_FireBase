...
function App() {
  ...
  // About Log-in
  useEffect(() => {
    onAuthStateChanged(authService, (user) => { // To cheack Any user logged in
      if (user) {
        setIsLoggedIN(true); // Logged-in-mode
        setUserObj({ // Setting logged-in User's Info
  ...
  // Common Button Class
  const btnClass = "bg-gradient-to-r from-indigo-300 to-indigo-400 transition shadow-md ease-in-out delay-70 bg-red-400 hover:-translate-y-2 duration-300'"

  return (
    <BrowserRouter>
      {navModal && <NavModal setNavModal={setNavModal} />} {/* NavModal function everywhere, Only When navModal is on */}
      {isLoggedIn && <NavBar setNavModal={setNavModal} userObj={userObj} />}{/* Navbar is always used, while logged in*/}
      <div id='container'>
        <Routes>
