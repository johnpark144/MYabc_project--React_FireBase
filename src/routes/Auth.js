...
export default function Auth() {
    ...
    const onSubmit = async (e) => {
        ...
        let data;
        try {
            if (newAccount) { // Create-account mode
        ...
        <div className={styles.container}>
            <div id={styles.authContainer} className='shadow-xl' onSubmit={onSubmit}>
                ...
                <form className={styles.loginSignup}>
                    <input className={styles.emailPassword} onChange={onChange}
                        required value={email} name="email" type='text' placeholder="Email" />
                    <input className={styles.emailPassword} onChange={onChange}
                        required value={password} name="password" type='password' placeholder="Password" />
                    <input id={styles.submitBtn} className='shadow-md' type='submit'
                        value={newAccount ? "Create Account" : "Sign in"} />
                    <span id={styles.error}>{error.slice(10, -1)}</span>
                </form>
                <div id={styles.toggleAccount} className='shadow-md' onClick={toggleAccount}>
                ...
                <div>
                    <div className={styles.snsAuth}>
                        <button onClick={onSocialClick} name="google" id={styles.googleAuthBtn} className='shadow-md'>
                            <i className='fa fa-google' style={{ marginRight: '10px' }} />
                            Continue with Google
                        </button>
                        <button onClick={onSocialClick} name="github" id={styles.githubAuthBtn} className='shadow-md'>
                            <i className='fa fa-github' style={{ marginRight: '10px' }} />
                            Continue with Github
                        </button>
                        <button onClick={onSocialClick} name="facebook" id={styles.facebookAuthBtn} className='shadow-md'>
                            <i className='fa fa-facebook-official' style={{ marginRight: '10px' }} />
