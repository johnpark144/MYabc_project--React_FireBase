...
export default function Create({ userObj, showCreate }) {
  ...
  return (
    ...
    >
      <div
        id={styles.allForm}
        className="shadow-lg bg-white w-64 m-auto"
        style={showCreate && { marginLeft: "60%" }}
      >
            ...
            <span className="flex items-center justify-between gap-4 w-full mt-8">
              ...
              {showCreate ? (
                ...
                <Link to={"/memorize"}>
                  <button
                    type="button"
                    className="py-2 px-4 bg-white hover:bg-gray-200 focus:ring-indigo-500 focus:ring-offset-indigo-200 text-indigo-500 text-gray w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2  rounded-lg "
                  >
                    Back
                  </button>
                </Link>
