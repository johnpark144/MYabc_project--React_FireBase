...
export default function Day({ userObj, btnClass }) {
    ...
    useEffect(() => {
        if (wordArr && dayArr) {
            // 1.
            setWords(wordArr.filter(word => (
                word.creatorId === userObj.uid && word.day === Number(day)
            ))); // Set words within same day
            // 2.
            setDays(dayArr.filter(day => (
        ...
        <div id='contents'>
            <div id={styles.menus} className="flex flex-row">
                ...
                <div className="grow h-14">
                    ...
                    <button onClick={togleHideEng} id={styles.hide}
                    ...
                    <button onClick={togleHideKor} id={styles.hide}
            ...
            <table id={styles.table}>
                ...
                <tbody>
                    {Array.isArray(words) ? (
                        words.map(word => (
                            <Word word={word} key={word.id} togleKor={togleKor} togleEng={togleEng}
                                userObj={userObj}
                            />
                        ))
                    ) : (
