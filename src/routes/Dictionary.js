    ...
    return (
        <div id='contents'>
            ...
            <div id={styles.search}>
                <input onChange={onChange} onKeyDown={onKeyDown} id={styles.searchBar}
                className="shadow-md shadow-purple-100/50" type='text' />
                <button onClick={search} id={styles.searchBtn}
                    className={btnClass}>
            ...
            <div>
                {wordInfo ? <>
                    ...
                    <div id={styles.wordAudio}>
                        <div>{wordInfo.word} {wordInfo.phonetic}</div>
                        <Audio />
                    </div>
                    {/* Deffinitions */}
                    <div id={styles.container}>{wordInfo.meanings.map((meaning, idx) => (
                        <div key={idx} id={styles.vocaContainer}
                        className='drop-shadow-lg font-serif bg-gradient-to-r from-purple-50 via-indigo-50 to-pink-50'>
                            ...
                            <div>
                                - synonym :
                                {meaning.synonyms[0] && meaning.synonyms.map((synonym, idx) => (
                                    <span key={idx}>
                                        {synonym},
                                    </span>
                                ))}
                            ...
                            <div>
                                - antonym :
                                {meaning.antonyms[0] && meaning.antonyms.map((antonym, idx) => (
                                    <span key={idx}>
                                        {antonym},
                                    </span>
                                ))}
                            ...
                            <div>
                                - definition :
                                <br />
                                {meaning.definitions[0] && meaning.definitions.map((definition, idx) => (
                                    <span key={idx}>
